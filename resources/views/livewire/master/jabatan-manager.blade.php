<div>
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h2 class="text-xl font-semibold text-slate-900">Master Data Jabatan</h2>
            <p class="mt-1 text-sm text-slate-500">Referensi posisi/jabatan perusahaan, sesuai struktur organisasi. Khusus posisi — tidak menyimpan nama orang.</p>
        </div>
    ...
    <x-card>
        <input type="text" wire:model.live.debounce.400ms="search" placeholder="Cari nama jabatan..." class="mb-4 w-full max-w-xs rounded-lg border-slate-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($items as $item)
                        <tr wire:key="{{ $item->id }}">
                            ...
                            <td class="px-5 py-3 text-slate-600">{{ $item->divisi->nama_divisi ?? '-' }}</td>
                            <td class="px-5 py-3">
                                @if ($item->is_active)
                            ...
                            <td class="px-5 py-3 text-right">
                                ...
                                <button wire:click="delete({{ $item->id }})" wire:confirm="Hapus jabatan ini?" class="text-xs font-medium text-rose-600 hover:underline">Hapus</button>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="4" class="px-5 py-10 text-center text-sm text-slate-400">Belum ada data jabatan.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
